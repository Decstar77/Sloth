...
namespace tower {
    ...
    void TowerWorld::Init( sloth::PhysicsWorld * newPhysicsWorld ) {
        physicsWorld = newPhysicsWorld;
    }

    i32 TowerWorld::AllocateSlot() {
        if ( !freeSlots.empty() ) {
    ...
    EntityId TowerWorld::SpawnEntity( Entity entity ) {
        SL_ASSERT_MSG( physicsWorld, "TowerWorld::Init() must be called before spawning entities" );
    ...
    void TowerWorld::DestroyEntity( EntityId id ) {
    ...
    Entity * TowerWorld::GetEntity( EntityId id ) {
    ...
    const Entity * TowerWorld::GetEntity( EntityId id ) const {
        return const_cast<TowerWorld *>( this )->GetEntity( id );
    }

    EntityId TowerWorld::FindEntityByRigidBody( sloth::RigidBody body ) const {
        if ( !body.IsValid() ) {
    ...
    void TowerWorld::ApplySpawn( const PendingSpawn & spawn ) {
    ...
    void TowerWorld::ApplyDestroy( EntityId id ) {
        if ( id.index < 0 || id.index >= static_cast<i32>( entities.size() ) ) {
            return;
        }
    ...
    void TowerWorld::FlushPendingChanges() {
