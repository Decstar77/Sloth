...
namespace sloth {
    ...
    //
    // Every widget takes a GuiFrame& (see sloth_gui_frame.h) rather than the
    // GuiContext/GuiRenderer/TextRenderer/Font/GlyphCache/viewProjection
    // tuple individually - build one with BeginGuiFrame() at the top of the
    // frame and thread it through every widget call that frame.
    ...
    // a content clip rect on both `frame.renderer` and `frame.ctx`; EndPanel pops
    // them. Draw panel content between the two calls.
    ...
    // the caller queued into `frame.renderer` (so it's drawn while the panel's
    // clip rect is still active) and pops the clip rect from both `frame.renderer`
    // and `frame.ctx`.
