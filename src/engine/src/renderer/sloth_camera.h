...
namespace sloth
{
    ...
    // A perspective camera. Holds position/orientation (as yaw/pitch, in
    // degrees) and lens parameters, and derives the view and projection
    // matrices used by the renderer. Movement/look behavior (free-fly,
    // orbit, etc.) is layered on top by the caller; this class is just the
    // math.
    class Camera
    {
    public:
        ...
        // Yaw/pitch in degrees. Yaw of -90 faces down -Z (GLM/OpenGL convention).
        void SetRotation(f32 newYawDegrees, f32 newPitchDegrees);
